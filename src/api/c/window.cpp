...
fg_err fg_draw_text(const fg_window pWindow, float pPos[], char* text,
...
fg_err fg_get_last_keyin(const fg_window pWindow, int* mLastKeyIn) {
    try {
        ARG_ASSERT(0, (pWindow != 0));
        *mLastKeyIn = getWindow(pWindow)->getLastKeyIn();
    }
    CATCHALL
    return FG_ERR_NONE;
}
