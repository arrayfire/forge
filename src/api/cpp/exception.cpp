...
exception::exception(const char *file, unsigned line, fg_err err)
{
    snprintf(m_msg, sizeof(m_msg)-1, "%s:%u: FG_ERROR %d", file, line, (int)(err));
    m_msg[sizeof(m_msg)-1] = '\0';
}

exception::exception(const char *msg, const char *file, unsigned line, fg_err err)
{
    snprintf(m_msg, sizeof(m_msg)-1, "%s\n%s:%u: FG_ERROR %d", msg, file, line, (int)(err));
    m_msg[sizeof(m_msg)-1] = '\0';
}
