...
#define FT_THROW_ERROR(msg, err)                        \
    ...
    void font_impl::loadAtlasWithGlyphs(const size_t pFontSize) {
        ...
        if (bError) {
            FT_Done_Face(face);
            FT_Done_FreeType(library);
            FT_THROW_ERROR("Freetype charmap set failed",
                           FG_ERR_FREETYPE_ERROR);
        }
        ...
        if (bError) {
            FT_Done_Face(face);
            FT_Done_FreeType(library);
            FT_THROW_ERROR("Freetype char size set failed",
                           FG_ERR_FREETYPE_ERROR);
        }
        ...
        for (int i = START_CHAR; i <= END_CHAR; ++i) {
            ...
            bError =
                FT_Glyph_To_Bitmap(&currGlyph, FT_RENDER_MODE_NORMAL, 0, 1);
            if (bError) {
            ...
            glyph->mAdvanceY = float((face->glyph->metrics.height -
    ...
    font_impl::font_impl()
        ...
        mGlyphLists.resize(size_t(MAX_FONT_SIZE - MIN_FONT_SIZE) + 1,
                           GlyphList());
    }
    ...
    void font_impl::loadFont(const char* const pFile) {
        ...
        for (size_t s = size_t(MIN_FONT_SIZE); s <= size_t(MAX_FONT_SIZE);
             ++s) {
    ...
    void font_impl::render(int pWindowId, const float pPos[],
                           const float pColor[], const char* pText,
        ...
        glm::mat4 R = (pIsVertical ? glm::rotate(I, glm::radians(90.f),
                                                 glm::vec3(0, 0, 1))
                                   : I);
        ...
        for (size_t i = 0; i < std::strlen(pText); ++i) {
            ...
            if (ccode >= START_CHAR && ccode <= END_CHAR) {
                ...
                glm::mat4 TR =
                    glm::translate(I, glm::vec3(loc_x, loc_y, 0.0f)) * R;
